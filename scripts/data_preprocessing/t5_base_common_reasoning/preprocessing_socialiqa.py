...
def preprocess_socialiqa(examples, tokenizer, config):
    ...
    model_inputs = tokenizer(
        ...
        max_length=config["preprocessing"]["max_length_stage2"],
        return_tensors="pt"
    )
    ...
    with tokenizer.as_target_tokenizer():
        labels = tokenizer(
            ...
            padding=config["preprocessing"]["padding"],
            truncation=config["preprocessing"]["truncation"],
            max_length=config["preprocessing"]["max_length_labels_stage2"],
            return_tensors="pt"
        )
...
def main():
    ...
    try:
        ...
        logging.error(f"Error during SocialIQA preprocessing: {e}")
        return
